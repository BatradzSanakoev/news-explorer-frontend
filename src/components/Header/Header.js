...
export default function Header({ loggedIn, username, location, openPopup, openBurger, isBurgerOpen, isLogPopupOpen, isRegPopupOpen }) {
    ...
    return (
        <header className={`header ${location.pathname === '/saved-news' ? 'header_auth' : ''} ${isBurgerOpen  && 'header_dark'}`}>
            <div className='header__container'>
                ...
                <Navigation loggedIn={loggedIn} username={username} location={location} openPopup={openPopup} isBurgerOpen={isBurgerOpen} openBurger={openBurger} isLogPopupOpen={isLogPopupOpen} isRegPopupOpen={isRegPopupOpen} />
            </div>
        </header>
    )
}